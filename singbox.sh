#!/bin/bash
# Sing-box 管理脚本 (部署/升级/卸载整合版)
# Author: Chis (优化 by ChatGPT)
# 集成：完整部署逻辑 + 升级 + 卸载 + 交互菜单
...
EOF
    ...
    # 开放防火墙端口
    if command -v ufw &>/dev/null && [[ $(ufw status | head -n1) != "Status: inactive" ]]; then
        ufw allow 80/tcp
        ufw allow 443/tcp
